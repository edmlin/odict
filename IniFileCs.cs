...
public class IniFile
{
	...
	public int GetInt(string sSection,string sKey,int def=0)
	{
		string value=GetKeyValue(sSection,sKey);
		int result;
		if(!Int32.TryParse(value,out result)) result=def;
		return result;
	}

	public float GetFloat(string sSection,string sKey,float def=0)
	{
		string value=GetKeyValue(sSection,sKey);
		float result;
		if(!float.TryParse(value,out result)) result=def;
		return result;
	}
	
	public bool GetBoolean(string sSection,string sKey,bool def=false)
	{
		string value=GetKeyValue(sSection,sKey);
		bool result;
		if(!Boolean.TryParse(value,out result)) result=def;
		return result;
	}
